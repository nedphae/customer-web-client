...
// 使用 input 标签获取，修复js执行顺序问题
// const userInfoText = document.getElementById("userInfo").value;
  ...
  axios.post('/access/customer/register', userInfo)
    .then((response) => {
      ...
      if (data && data.errorCode === undefined) {
        ...
        config.messages = convId ? historyMsg.map((msg) => {
          if (msg.type === "image") {
        ...
        config.quickReplies = [
          {
            ...
            card: {
              ...
              data: {
                ...
                userInfo: { organizationId: organizationId, shuntId: shuntId, userId: userId, uid: userInfo.uid, referrer: userInfo.referrer, title: userInfo.title }
              }
            }
          },
          {
            ...
            card: {
              ...
              data: {
                ...
                convInfo: { organizationId: organizationId, convId: convId }
              }
            }
          },
        ].concat(config.quickReplies ?? [])

        if (historyMsg[0] && historyMsg[0]._id) {
        ...
        bot = new ChatSDK({
          ...
          requests: {
            ...
            history: function () {
              return {
                url: '/access/customer/message/history?oid=' + organizationId + '$userId=' + userId + '&lastSeqId=' + lastMsgId + '&pageSize=50',
              };
            },
            send: function (msg) {
              // 机器人配置
        ...
        if (!!convId) {
          ...
          ctx.appendMessage({
            type: 'system',
            content: {
              text: '当前无任何客服在线，您可以稍后再试，或者点击下方留言按钮留言',
            },
          });
        }
        ...
        div.innerHTML = '<div style="text-align: center; padding-top: 50px;">没有找到接待组</div>';
      }
    })
} else {
  console.error('请配置机器人配置信息');
}
